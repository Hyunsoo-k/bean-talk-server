...
const checkNotificationMiddleware = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  ...
  const notification = notificationContainer.notifications.id(notification_id);
  if (!notification) {
...
export default checkNotificationMiddleware;
